...
async function generateFeed(propertyId, propertyName, feedId) {
  try {
    ...
    for (const b of bookings) {
      ...
      // every connected platform. Found in production 2026-07-29: this
      // exact property had a 'blocked' row spanning 2026-02-01 to
      // 2026-12-31 that did exactly that on LekkeSlaap. Skip anything
      // over ~4 months instead.
    ...
    if (emitted === 0) {
      ...
      lines.push('BEGIN:VEVENT');
      lines.push('UID:hep-tvhouse-placeholder@snapartments.co.za');

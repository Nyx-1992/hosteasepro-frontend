...
| 090_staging_ical_feeds_seed.sql | **staging only** | Seeds `ical_feeds` on staging so iCal sync can be smoke-tested there. Since p1-1, this table is the live source of truth for both the client (`loadIcalFeeds()`) and the background cron (`scripts/ics-import.js`) — no longer just a testing seed. Since B4, `last_import_at` (a column that already existed but was never written to) is updated by both sync paths on every successful fetch, and read back by the dashboard's sync-health indicator — no schema change, just the column finally being used |
...
| 160_cleanup_ical_horizon_junk.sql | both, **including production** | **Incident cleanup — caveat below.** Deleted 73 stale duplicate `'blocked'` rows on TV House's booking.com feed (Booking.com re-issues a long-running closure with a fresh UID on every fetch; the old sync logic couldn't recognize it as the same closure and kept inserting new rows). Already run everywhere; **do not re-run on a fresh environment** — its `DELETE` doesn't filter `is_active`, so on a database where a real closure has since grown past 120 nights it would delete the active row (harmless in practice — the next sync recreates it via `syncICalFeeds`'s overlap-matching fallback, added directly in `demo/index_fixed.html`, no migration — but avoid it anyway) |
...
| 180_roadmap_state_p1_4.sql | both | Marks Roadmap item p1-4 (per-org commission rates) done — satisfied by Task 5's platform_fee_config table |
...
| 510_roadmap_state_p0_25_p0_29.sql | both | Marks Roadmap items p0-25 ("Edit clean" not saving a date change — confirmed already fixed by p0-39) and p0-29 (fmtDate() full-timestamp bug — confirmed already fixed by the bookings-array normalizer) done. No code change, verification only |

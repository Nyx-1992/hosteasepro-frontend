...
// Declare Deno for TypeScript tooling outside Deno runtime
...
async function upsertBookings(rows: any[]) {
  ...
  for (let i=0; i<rows.length; i+=batchSize) {
    const slice = rows.slice(i, i+batchSize);
      ...
      method: 'POST',
      headers: { apikey: SUPABASE_SERVICE_KEY!, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, Prefer: 'resolution=merge-duplicates,return=minimal', 'Content-Type': 'application/json' },
